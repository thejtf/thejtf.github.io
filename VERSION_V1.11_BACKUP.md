...
#### 日期/时间格式
- **日期格式**: `YYYY-MM-DD` (例如: 2025-12-29) - 用于文章列表中的日期显示
- **时间格式**: `HH:mm:ss`
- **更新选项**: mtime
- **注意**: location-bar 中的日期显示使用 JavaScript 动态计算，格式为 `Sunday December 29`（不含年份）

#### RSS Feed 配置（V1.11 新增）
- **类型**: atom
- **路径**: atom.xml
- **文章数量限制**: 20
- **RSS 标题**: Jopus的博客
...
**修改前**：
- RSS 标题：使用站点配置的 `title`（"Jopus"）
- RSS 图标：无
- 配置项：`feed_generator`（错误）

**修改后**：
- RSS 标题：`Jopus的博客`（通过 `feed.title` 配置）
- RSS 图标：`https://jopus.cn/favicon.ico`（通过 `feed.icon` 配置）
- 配置项：`feed`（正确）

**修改文件**：
- `_config.yml` - 添加 `feed` 配置项（第112-119行）
- `source/_atom.xml` - 创建自定义 Atom XML 模板（新文件）

**技术细节**：
- 配置项：`feed`（不是 `feed_generator`）
- 自定义模板：`./source/_atom.xml`
- 模板修改：`{{ config.title }}` → `{{ config.feed.title or config.title }}`
- RSS 图标：在 Atom XML 中通过 `<icon>` 标签添加
...
feed:
  ...
  limit: 20
  title: Jopus的博客
  icon: https://jopus.cn/favicon.ico
  template: ./source/_atom.xml
```

**生成的 RSS Feed 内容**：
- `<title>Jopus的博客</title>`
- `<icon>https://jopus.cn/favicon.ico</icon>`
...
cd /home/jopus/Blog
tar -xzf /home/jopus/Backup/Blog_V1.11_Backup_20251229.tar.gz
...
# 或者创建新分支基于V1.11
git checkout -b restore-v1.11 v1.11
...
5. **RSS Feed 配置**: 
   ...
   - RSS 图标：`https://jopus.cn/favicon.ico`
6. **日期格式**: 
   - location-bar（首页日期）: `Sunday December 29` - JavaScript 动态计算，不含年份
   - post__date（文章日期）: `2025-12-29` - 使用 `YYYY-MM-DD` 格式
7. **Favicon 文件**: 确保 `themes/paper/source/favicon.ico` 文件存在且为最新版本
8. **字体加载**: 字体加载已从 `_config.yml` 移动到 `layout.pug`，使用优化的预连接和预加载策略
9. **侧边栏边框**: 侧边栏左侧边框使用浅灰色（`lighten($color-gray, 70%)`），与文章列表项下边框样式一致

---

## 📌 版本快照说明

此文档记录了项目在 V1.11 版本时的完整配置状态，包括：
- ✅ 所有依赖包的版本号
- ✅ 主要配置文件的参数设置
- ✅ 主题配置信息
- ✅ 部署相关配置
...
- ✅ 日期格式配置（post__date 使用 YYYY-MM-DD，location-bar 使用 JavaScript 显示 Sunday December 29）
- ✅ Favicon 文件更新
- ✅ 侧边栏按钮UI修复
- ✅ 字体加载优化（预连接、预加载、异步加载）
...
**建议**: 在升级到新版本前，请确保已保存此备份文档、Git标签和压缩包备份，以便需要时能够快速恢复到 V1.11 版本。

### 压缩包内容说明

压缩包 `/home/jopus/Backup/Blog_V1.11_Backup_20251229.tar.gz` 包含以下内容：
- ✅ `source/` - 所有源文件（文章、页面等，包含 `_atom.xml` 自定义模板）
- ✅ `themes/` - 完整主题文件（包含动态主题色功能、更新的 favicon.ico、侧边栏按钮修复、字体加载优化和侧边栏边框样式统一）
- ✅ `scaffolds/` - 模板文件
- ✅ `_config.yml` - 主配置文件（包含 RSS Feed 配置）
- ✅ `package.json` - 依赖配置
...
- **备份文档**: `VERSION_V1.11_BACKUP.md`（本文件）
- **Git标签**: `v1.11`（待创建）
- **压缩包备份**: `/home/jopus/Backup/Blog_V1.11_Backup_20251229.tar.gz`（待创建）
...
- RSS 标题：`Jopus的博客`（通过 `feed.title` 配置）
- RSS 图标：`https://jopus.cn/favicon.ico`（通过 `feed.icon` 配置）
- 自定义模板：`source/_atom.xml`（支持 `feed.title` 配置）

#### 技术细节
- **配置项名称**：`feed`（不是 `feed_generator`）
- **自定义模板**：`./source/_atom.xml`
- **模板修改**：`{{ config.title }}` → `{{ config.feed.title or config.title }}`
- **RSS 图标**：在 Atom XML 中通过 `<icon>` 标签添加
- **生成的 RSS Feed**：
...
#### 修改原因
- 用户要求 RSS 标题为 "Jopus的博客"（与网站标题区分）
- 用户要求添加 RSS favicon 图标

---

## 🎯 V1.11 核心功能

### RSS Feed 配置优化（V1.11 新增）
