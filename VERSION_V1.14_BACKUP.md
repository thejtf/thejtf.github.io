...
#### 日期/时间格式
- **日期格式**: `YYYY-MM-DD` (例如: 2026-03-03) - 用于文章列表中的日期显示
- **时间格式**: `HH:mm:ss`
- **更新选项**: mtime
- **注意**: location-bar 中的日期显示使用 JavaScript 动态计算，格式为 `Monday March 3`（不含年份）
...
### V1.14 主要更新（2026年03月03日）

#### 1. Footer 版权链接移除
- ✅ **移除 Jopus 链接**: 将 footer 中的 "Jopus" 从可点击链接改为纯文本
- ✅ **保留样式**: 保留 `<strong>` 加粗样式

**修改前（V1.13）**：
```pug
.footer
  span ©️2020-#{new Date().getFullYear()} Created By&nbsp;
    strong
      a(href='https://x.com/JopusLee') Jopus
    |
```
...
cd /home/jopus/Blog
tar -xzf /home/jopus/Backup/Blog_V1.14_Backup_20260303.tar.gz
...
# 或者创建新分支基于V1.14
git checkout -b restore-v1.14 v1.14
...
# 4. 恢复 footer 文件
# 确保 themes/paper/layout/includes/footer.pug 中 Jopus 无链接
...
**建议**: 在升级到新版本前，请确保已保存此备份文档、Git标签和压缩包备份，以便需要时能够快速恢复到 V1.14 版本。

### 压缩包内容说明

压缩包 `/home/jopus/Backup/Blog_V1.14_Backup_20260303.tar.gz` 包含以下内容：
- ✅ `source/` - 所有源文件（文章、页面等）
- ✅ `themes/` - 完整主题文件
- ✅ `scaffolds/` - 模板文件
...
- **备份文档**: `VERSION_V1.14_BACKUP.md`（本文件）
- **Git标签**: `v1.14`（待创建）
- **压缩包备份**: `/home/jopus/Backup/Blog_V1.14_Backup_20260303.tar.gz`（待创建）
...
#### 修改文件
- `themes/paper/layout/includes/footer.pug`
...
- 版权文字：`©️2020-2026 Created By Jopus`
- Jopus 为纯文本，无链接
- 保留加粗样式
