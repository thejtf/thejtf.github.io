...
cd /home/jopus/Blog
tar -xzf /home/jopus/Backup/Blog_V1.07_Backup_20251226.tar.gz
...
# 或者创建新分支基于V1.07
git checkout -b restore-v1.07 v1.07
...
# 4. 恢复 favicon.ico
# 将备份的 themes/paper/source/favicon.ico 复制到对应位置
...
**建议**: 在升级到新版本前，请确保已保存此备份文档、Git标签和压缩包备份，以便需要时能够快速恢复到 V1.07 版本。

### 压缩包内容说明

压缩包 `/home/jopus/Backup/Blog_V1.07_Backup_20251226.tar.gz` 包含以下内容：
- ✅ `source/` - 所有源文件（文章、页面等）
- ✅ `themes/` - 完整主题文件（包含动态主题色功能和更新的 favicon.ico）
- ✅ `scaffolds/` - 模板文件
- ✅ `_config.yml` - 主配置文件
- ✅ `package.json` - 依赖配置
...
- **备份文档**: `VERSION_V1.07_BACKUP.md`（本文件）
- **Git标签**: `v1.07`（待创建）
- **压缩包备份**: `/home/jopus/Backup/Blog_V1.07_Backup_20251226.tar.gz`（已创建）
...
#### 文件位置
- **源文件**: `themes/paper/source/favicon.ico`
- **生成文件**: `public/favicon.ico`（Hexo 自动生成）

#### 更新内容
- 从 Mac 桌面传输新的 favicon.ico 文件到主题目录
- 文件大小：16KB
- 在浏览器标签页显示新的网站图标
